...
## to calculate an inverse of a matrix stored in makeCacheMatrix we need to go via cacheSolve function
...
makeCacheMatrix <- function(x = matrix()) {
        inv <- NULL
        ## set function - stores new matrix in the makeCacheMatrix environment
        set <- function(y) {
...
## cacheSolve function returns an inverse of a matrix stored in makeCacheMatrix environment
## if an inverse of matrix exists in makeCacheMatrix environment (cache) then it will be returned
## else an inverse will be calculated, stored in makeCacheMatrix environment (cached) and returned
## argument x is a list returned by makeCacheMatrix function
## we need to call makeCacheMArix before calling cacheSolve
cacheSolve <- function(x, ...) {
        ## x$getinv() returns an inverse of a matrix stored in makeCacheMatrix environment, if an inverse was not calculated yet then NULL is returned
        inv <- x$getinv()
        if(!is.null(inv)) {
            message("getting cached matrix from makeCacheMatrix environment")
            return(inv) ## inverse returned and function exits here
        }
        ## the below code runs only if we do not have an inverse in the makeCacheMatrix environment (inverse was not-cached yet)
